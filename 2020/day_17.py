...
class ConwayCube():
	...
	def __active_neighbors(self, coordinate):
		...
		for i in itertools.product(*temp):
			...
			if not is_zero and [ii[j] + coordinate[j] for j in range(self.dimension)] in self.active:
				count += 1
	...
	def cycle(self, num):
		...
		temp = [range(self.range[i][0] - 1, self.range[i][1] + 2) for i in range(self.dimension)]
